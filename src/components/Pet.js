...
function Pet({pet, adoptPet, updatePet}){
    ...
    return (
        <li className="pet">
            ...
            <h4 onClick={()=>setDisplayName(!displayName)}>{displayName? pet.name: pet.animal_type}</h4>
            <button onClick={()=> updatePet(pet)} className="likes-button">{pet.likes} likes</button>
            <br/><br/>
            <button onClick={() => {
                adoptPet(pet.id)
